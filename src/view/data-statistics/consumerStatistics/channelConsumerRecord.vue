    ...
    <nav>
      ...
      <el-col :span="24">
        <el-table :data="tableData"  align='left' highlight-current-row row-class-name="cell-style" style="width: 100%">
          ...
          <el-table-column prop="consumeUser" label="消费用户数" >
          </el-table-column>
      
         <!--  <el-table-column prop="vipLevel" label="新增消费用户数" >
          </el-table-column> -->
          <el-table-column prop="consumeDimanond" label="消费钻石数" >
          </el-table-column>
          <el-table-column prop="consumeCount" label="消费次数" >
          </el-table-column>
          <el-table-column prop="userAvgConsume" label="人均消费次数" >
          </el-table-column>
        </el-table>
      </el-col>
<el-col :span="24">
...
export default {
  ...
  methods: {
     beginDownExcl(title){
           let keytitle  = {
            ...
            channel:'渠道',
            consumeUser:'消费用户数',
            consumeDimanond:'消费钻石数',
            consumeCount:'消费次数',
            userAvgConsume:'人均消费次数',
           } ,vm = this;
         title = title||'表格'    
          this.exportLists('ApiconsumeStatisticschannel').then((res)=>{
  ...
  queryRechargeLists() {//用户
      ...
      option.endTime = hsdateUtil(option.endTime)
                this.API.ApiconsumeStatisticschannel(option).then((res)=>{
