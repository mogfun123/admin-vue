     ...
     <!--  <el-row>
      ...
      <el-col :span="24">
        <el-table :data="tableData"  align='left' highlight-current-row row-class-name="cell-style" style="width: 100%">
         <el-table-column prop= 'channel' label="用户渠道" >
          </el-table-column>
          ...
          <el-table-column prop="sweepTotal" label="好友进入分享页次数" >
          </el-table-column>
          <el-table-column prop="downloadTotal" label="好友下载游戏次数" >
          </el-table-column>
           <el-table-column prop="transformRate" label="下载转化率" >
          </el-table-column>
        </el-table>
      </el-col>
<el-col :span="24">
...
export default {
  ...
  methods: {
      ...
      beginDownExcl(title){
         ...
         title = title||'表格'
          this.exportLists('ApishareSweepchannel').then((res)=>{
  ...
  queryRechargeLists() {//用户
      ...
      option.endTime = hsdateUtil(option.endTime)
                this.API.ApishareSweepchannel(option).then((res)=>{
